...
train_imgs = [x for x in range(   1,   1+9000)]
...
def getNeatSentns(dirtySentns):
    tokens=jieba.cut(dirtySentns)
    neatSentns = ""
    for token in tokens:
        # 检测该词是否和过滤集有交集
        if set(token) & filterSet != set() :# 如果有，这词是标点符号，扔掉
            continue
        else: # 非标点符号
            neatSentns += " "+token
            vocabSet.add(token)
    return neatSentns.strip()
...
for line in open(captionTrainFile, 'r'):
    ...
    if line.isdigit(): # Meet a new image
        imgID=int(line)
        continue
    neatSentence = getNeatSentns(line)   
    try:
