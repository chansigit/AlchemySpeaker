...
def test_model(weight, img_name, beam_size = 3):
    encoded_images = pickle.load( open( "temp/encoded_images.p", "rb" ) )
...
def test_model_on_images(weight, img_dir, beam_size = 3):
    ...
    f_pred_caption = open('SubmitingResult.txt', 'w')
    for count, img_name in enumerate(imgs):
        ...
        best_caption=str(best_caption)
        best_caption="".join(best_caption.split())
        imgTitle = str(int(img_name)-1)+" "
        f_pred_caption.write(imgTitle+str(" ".join(best_caption))+"\n")
        if count%20==0:
...
if __name__ == '__main__':
    weight = 'weights-improvement-09.hdf5'
    test_img_dir = 'Automation.subImages.txt'
