from MagicSpeaker.CaptionGenerator import CaptionGenerator
import pickle
...
def get_best_caption(captions):
    ...
    result= " ".join([cg.index_word[index] for index in best_caption])
    #print(result)
    return result
...
def test_model_on_images(weight, imgIDList, beam_size = 3):
    refCaptions = pickle.load(open("captionData.pkl","rb"))
    captions = {}
    encoded_images = pickle.load( open( "encoded_images.p", "rb" ) )
    model = cg.create_model(ret_model = True)
    model.load_weights(weight)
    ...
    for imgID in imgIDList:
        print("Predicting for image: %d" % imgID)
        image = encoded_images[imgID]
        image_captions = generate_captions(model, image, beam_size)
        
        best_caption = process_caption(get_best_caption(image_captions))
        captions[imgID] = best_caption
        print("%d : %s\n> %s"% (imgID, str(best_caption), refCaptions[imgID])  )
        
        f_pred_caption.write("%d\t%s"% (imgID, str(best_caption)) )
    ...
    references = []
    for imgID in imgIDList:
        hypothesis = captions[imgID]
        reference = refCaptions[imgID]
...
print(test_model_on_images(weight, test_imgs, beam_size=3))
